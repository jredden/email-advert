...
//			logger.error("Empty Connector implementation class name provided");
			System.out.println("Empty Connector implementation class name provided");
		else {
			try {
...
//		logger.debug("Entering Dispatcher#doPost");
		System.out.println("Entering Dispatcher#doPost");
		...
		if (!RequestCycleHandler.isFileUploadEnabled(request)){
			...
			System.out.println("UploadResponse.getFileUploadDisabledError");
		}
		// check parameters  
		else if (!Command.isValidForPost(context.getCommandStr())){
			uploadResponse = UploadResponse.getInvalidCommandError();
			System.out.println("UploadResponse.getInvalidCommandError");
		}
		else if (!ResourceType.isValidType(context.getTypeStr())){
			uploadResponse = UploadResponse.getInvalidResourceTypeError();
			System.out.println("UploadResponse.getInvalidResourceTypeError");
		}
		else if (!UtilsFile.isValidPath(context.getCurrentFolderStr())){
			uploadResponse = UploadResponse.getInvalidCurrentFolderError();
			System.out.println("UploadResponse.getInvalidCurrentFolderError");
		}
		else {
			...
			try {
				...
				System.out.println("uplFile:"+uplFile);
				// Some browsers transfer the entire source path not just the
				// filename
				fileName = FilenameUtils.getName(uplFile.getName());
...
//								newFileName);
				...
				uploadResponse = UploadResponse.getInvalidCurrentFolderError();
				System.out.println("UploadResponse.getInvalidCurrentFolderError2");
			} catch (WriteException e) {
				uploadResponse = UploadResponse.getFileUploadWriteError();
				System.out.println("UploadResponse.getFileUploadWriteError1");
			} catch (IOException e) {
				uploadResponse = UploadResponse.getFileUploadWriteError();
				System.out.println("UploadResponse.getFileUploadWriteError2");
			} catch (FileUploadException e) {
				uploadResponse = UploadResponse.getFileUploadWriteError();
				System.out.println("UploadResponse.getFileUploadWriteError3");
			}
		}
